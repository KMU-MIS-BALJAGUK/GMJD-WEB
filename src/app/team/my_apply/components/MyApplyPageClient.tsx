...
export default function MyApplyPageClient() {
  ...
  const normalizeRecruitStatus = (
    ...
    if (primary === "\\ubaa8\\uc9d1\\uc911" || primary === "OPEN") return "\\ubaa8\\uc9d1\\uc911";
    return "\\ubaa8\\uc9d1\\uc644\\ub8cc";
  };
  ...
  return (
    <>
      <div className="max-w-[1200px] mx-auto max-md:py-7 py-10 px-4 md:px-6 lg:px-8">
        <h1 className="max-md:text-xl text-2xl font-bold mb-6">? ??</h1>
        ...
        {!isLoading && myAppliedList?.length === 0 && (
          <div className="flex flex-col items-center justify-center h-[300px] text-center">
            ...
            <p className="text-gray-700 font-medium text-sm">??? ?? ?? ???.</p>
        ...
        {myAppliedList && myAppliedList.length > 0 && !isLoading && !isError && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {myAppliedList.map((item) => {
              const recruitStatus = normalizeRecruitStatus(item.recruitStatus, item.status);
